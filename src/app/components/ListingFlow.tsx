...
import CreateItemLayout from "./CreateItemLayout";
...
export default function ListingFlow() {
  ...
  const [shouldCollapseItemDetails, setShouldCollapseItemDetails] = useState(false);
  const [shouldCollapseMarketplaces, setShouldCollapseMarketplaces] = useState(false);
  ...
  const [marketplaceCustomizations, setMarketplaceCustomizations] = useState<Record<string, MarketplaceCustomization>>({});
  // Lifted marketplace preview dialog state (so dialog renders outside overflow containers)
  const [previewMarketplace, setPreviewMarketplace] = useState<Marketplace | null>(null);
  ...
  const { navigateToSection, navigateToReview, navigateToSuccess, navigateToPhotos, navigateToPublish } =
    useListingRouteSync({
      isReviewMode,
      isPublished,
      isSideSheetOpen,
  ...
  const collapseAllExcept = (except: 'photos' | 'itemDetails' | 'marketplaces' | 'pricing' | 'shipping') => {
    ...
    <div className="flex h-screen w-screen min-w-[1280px] flex-col">
                  ...
                  >
                    <div className="overflow-clip relative shrink-0 size-[20px]">
                      <svg className="block size-full" fill="none" viewBox="0 0 20 20">
                        <path d="M12.5 15L7.5 10L12.5 5" stroke="var(--muted-foreground)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
                    <p className="font-['Lexend',sans-serif] font-[var(--font-weight-medium)] leading-[20px] text-muted-foreground text-[var(--text-sm)] tracking-[0.1px]">Back to Editing</p>
                  </button>
                  <div className="flex-1" />
                  <p className="font-['Lexend',sans-serif] font-[var(--font-weight-normal)] leading-[28px] text-foreground text-[var(--text-h4)]">Review Listing</p>
                  <div className="flex-1" />
              ...
              <div className="flex-1 min-w-0">
                <CreateItemLayout
                  ...
                  children={{
                    ...
                    itemDetails: <ItemDetailsContent initialData={aiGeneratedDetails} shouldExpand={shouldExpandItemDetails} onExpandChange={() => setShouldExpandItemDetails(false)} onContinue={handleContinueFromItemDetails} onDetailsChange={setItemDetails} shouldCollapse={shouldCollapseItemDetails} onCollapseChange={() => setShouldCollapseItemDetails(false)} onManualExpand={handleManualExpandItemDetails} />,
                    marketplaces: <MarketplacesContent shouldExpand={shouldExpandMarketplaces} onExpandChange={() => setShouldExpandMarketplaces(false)} onContinue={handleContinueFromMarketplaces} selectedMarketplaces={selectedMarketplaces} onMarketplacesChange={setSelectedMarketplaces} shouldCollapse={shouldCollapseMarketplaces} onCollapseChange={() => setShouldCollapseMarketplaces(false)} onManualExpand={handleManualExpandMarketplaces} />,
                    pricing: <PricingContent isAIGenerated={wasAIGeneratedFromPhotos} hasUsedAI={wasAIGeneratedFromPhotos} shouldExpand={shouldExpandPricing} onExpandChange={() => setShouldExpandPricing(false)} onContinue={handleContinueFromPricing} onPriceChange={setListingPrice} initialPrice={itemDetails?.suggestedPrice ? itemDetails.suggestedPrice.replace(/[^0-9.]/g, '') : undefined} shouldCollapse={shouldCollapsePricing} onCollapseChange={() => setShouldCollapsePricing(false)} onManualExpand={handleManualExpandPricing} />,
                    shipping: <ShippingContent isAIGenerated={wasAIGeneratedFromPhotos} shouldExpand={shouldExpandShipping} onExpandChange={() => setShouldExpandShipping(false)} shouldCollapse={shouldCollapseShipping} onCollapseChange={() => setShouldCollapseShipping(false)} onManualExpand={handleManualExpandShipping} onContinue={handleContinueFromShipping} uploadedPhotos={uploadedPhotos} onShippingMethodChange={setSelectedShippingMethod} />,
                  }}
                />
              </div>

              {/* Vertical Divider */}
              <div className="sticky top-0 h-screen w-[1px] bg-border z-[10] mx-[-4px] my-[0px]" style={{ left: 'calc(100% - 320px)' }} />
      ...
      {previewMarketplace && (
        <MarketplaceListingPreviewDialog
          ...
          marketplaceCustomization={marketplaceCustomizations[previewMarketplace.id]}
        />
      )}
    </div>
  );
